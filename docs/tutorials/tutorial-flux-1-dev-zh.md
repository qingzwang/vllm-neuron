...
两边在"该怎么定容量"这件事上的结论是一致的：已经在设备上的 adapter 免费——**包括在两个都常驻的
adapter 之间切**，NxDI 把 `max_loras` 开到装得下之后同样是 +0.4 ms——不在设备上的那部分代价才是
搬数据。NxDI 那个 1.84 s 属于后一种情形，不是前一种：决定一个请求落在哪一种的是它的 `max_loras`，
和这边的 `lora_slots` 是同一个旋钮。第四行的绝对差是机制上的——NxDI 重写槽位是约 4300 次小拷贝，这边写的是 990 个在 host
上补好零的矩阵——也正因为如此，两边都应该把 `lora_slots` 开到覆盖热点集合。

#### 多个 adapter 同时常驻

这才是槽位想要的用法：编译时把槽位声明出来，启动时每个 adapter 装一次，之后只 `set_lora()`。
5 个 adapter、1024×1024、`tp_degree=4`、`lora_max_rank=64` 实测：

| | |
|---|---|
| 槽位显存 | 385 MB / 槽 / rank，5 个就是每 rank 1.93 GB，叠在 7.86 GiB 权重之上 |
| 启动时把 5 个全装进去 | **1.31 s**（22 MiB 的 0.13 s，585 MiB 的 0.47 s） |
| 5 个各自生效时的 ms/step | 302.0–303.9，底模是 303.8 |
| `set_lora()` 在 5 个槽位间轮转 | p50 0.28 ms、p90 0.66 ms、max 1.4 ms |
| 每个请求换一个 adapter | 2482 ms / 8 步请求，对比从不换的 2477 ms |

**除了显存，没有任何东西随槽位数增长。** 一步只通过 `index_select` 读一个槽位，不管它是第几个，
所以 5 个常驻 adapter 的每步开销和 2 个一样，5 个之间切换的代价也和 2 个之间一样。

于是定容量的算术很简单：`lora_slots` 按你要服务的 adapter 数来定，再拿 `lora_slots × 385 MB`
去对每个核的余量（约 22 GiB，减掉 7.86 GiB 权重和激活）。`lora_max_rank=16` 时一个槽位只要
96 MB——所以编译前先搞清楚你真正会用的最大 rank。
...
槽位 0 是底模，而且一直是底模：两个 adapter 都加载并用过之后，切回槽位 0 复现了第一张底模图，
逐位相同。但它和**不带槽位编译出来的模型**不是逐位相同的——4 步之后 cos 0.999835、max|d| 0.31
（latent 标准差 1.12）。零槽位贡献的是精确的零，所以差异不来自 delta，而是多出来的算子改变了编译器
对整张图的调度顺序，性质和换 `tp_degree` 一样。**所以"同种子可复现"的范围是同一套构建配置，
`lora_slots` 也算在里面。**
