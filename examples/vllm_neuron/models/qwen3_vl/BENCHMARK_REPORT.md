...
| **TP=2 × DP=2（修复 QKV kernel 后，正确）** | 16 | 1626 | 47.45 | **13485** | **286.1** | 1.12 |

如果只看吞吐表，会得出「TP=2 × DP=2 比 TP=4 快 9.4%，突破了 batch 16 悬崖」的结论 ——
**完全错误，因为算出来的东西是错的**。这些数字全部作废。

**修复 QKV kernel 后重跑（表格最后一行），真实数字是 286.1 tok/s vs TP=4 的 284.1 ——
打平（+0.7%，噪声内），而端到端延时翻倍（13.5 s vs 6.9 s）。** 乱码版本承诺的 +9.4% 是假的。
这否掉了「小 TP + 多副本」提升吞吐天花板的整条思路：修好之后它能算对，但什么也没换到。
与事前的权重流量推算一致 —— DP 复制权重，同等并发下 TP 的分片让一份权重服务整个 batch。
...
| ~~TP=2 × DP=2（修复后）~~ | **已测，打平** —— 286.1 vs 284.1 tok/s，延时翻倍（§6.4） | — |
