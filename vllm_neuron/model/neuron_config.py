...
class NeuronConfig:
    ...
    # do not yet report MAC counts (see the FIXME in
    # ``NeuronModelRunner.load_model``). Set ``""`` to pass no extra options,
    # which is what a model built entirely from torch ops wants: the threshold
    # makes neuronx-cc fail codegen on some such graphs (Qwen3.5's decode graph
    # hits NCC_IBTN006, a pftranspose whose copy fails backend verification).
